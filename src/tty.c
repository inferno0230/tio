...
static int tty_baudrate_index_find(int baudrate)
{
    for (int i=0; i<option.baudrates_count; i++)
    {
        if (option.baudrates[i] == baudrate)
        {
            return i;
        }
    }
...
static void tty_baudrate_cycle(void)
{
    ...
    int old_index = tty_baudrate_index_find(option.baudrate);
    int new_index;
    ...
    if (old_index < 0)
    {
        old_index = option.baudrate_index;
    }
    ...
    else
    {
        ...
        tty_baudrate_apply(old_baudrate);
    }
}
...
static readability_t tty_sample_readability(const unsigned char *buffer, size_t length)
{
    ...
    if ((bad >= 8) && ((bad * 100 / length) >= 25))
    {
        return READABILITY_GARBLED;
    }

    if ((good * 100 / length) >= 85)
    {
...
static bool tty_auto_baud_observe(const char *buffer, size_t length)
{
    size_t offset = 0;
...
static ssize_t tty_auto_baud_read_probe(unsigned char *buffer, size_t length)
{
    ...
    while ((total < (ssize_t)length) && (timeval_elapsed_ms(&start) < AUTO_BAUD_PROBE_TIMEOUT_MS))
    {
        long remaining = AUTO_BAUD_PROBE_TIMEOUT_MS - timeval_elapsed_ms(&start);
...
static void tty_auto_baud_print_banner(int old_baudrate, int new_baudrate)
{
    tio_printf("==================================");
    tio_printf("[tio] AUTO BAUD CHANGED: %d -> %d", old_baudrate, new_baudrate);
    tio_printf("==================================");
}

static void tty_auto_baud_output_probe(char *input_buffer, ssize_t bytes_read, bool *do_timestamp)
{
    char input_char;
    char* now = NULL;
    ...
    for (int i=0; i<bytes_read; i++)
    {
        ...
        if (!option.map_i_msb2lsb &&
            (((input_char == '\n') && option.map_i_nl_crnl) ||
             ((input_char == '\r') && option.map_i_cr_crnl)))
        {
            printchar('\r');
            printchar('\n');
            if (option.timestamp)
...
static bool tty_auto_baud_probe(bool *do_timestamp)
{
    ...
    for (int i=1; i<option.baudrates_count; i++)
    {
        ...
        bytes_read = tty_auto_baud_read_probe(probe_buffer, sizeof(probe_buffer));
        if (bytes_read < 0)
        {
            break;
        }
        ...
        {
            tty_auto_baud_print_banner(old_baudrate, baudrate);
            tty_auto_baud_output_probe((char *)probe_buffer, bytes_read, do_timestamp);
            tty_auto_baud_sample_reset();
            return true;
        }
    }
    ...
    while (true)
    {
        ...
        if (status > 0)
        {
            ...
            if (FD_ISSET(device_fd, &rdfs))
            {
                ...
                {
                    tty_auto_baud_probe(&do_timestamp);
                    continue;
                }
