...
#define AUTO_BAUD_TRIGGER_SAMPLE_SIZE 128
#define AUTO_BAUD_MIN_SAMPLE_SIZE 32
#define AUTO_BAUD_MIN_BAD_BYTES 8
#define AUTO_BAUD_GARBLED_THRESHOLD_PCT 25
#define AUTO_BAUD_READABLE_THRESHOLD_PCT 85
#define AUTO_BAUD_PROBE_SAMPLE_SIZE 512
/* Total probing budget; long enough for typical boot logs to emit more text. */
#define AUTO_BAUD_PROBE_TIMEOUT_MS 700
#define AUTO_BAUD_FAILED_PROBE_COOLDOWN_MS 2000
...
static unsigned char auto_baud_sample[AUTO_BAUD_TRIGGER_SAMPLE_SIZE];
static size_t auto_baud_sample_count = 0;
static struct timeval auto_baud_failed_probe_at = {};
static bool auto_baud_failed_probe_pending = false;

static int tty_baudrate_apply(int baudrate);
static void tty_baudrate_cycle(void);
...
static void tty_baudrate_cycle(void)
{
    int old_baudrate = option.baudrate;
    int old_index = option.baudrate_index;
    int new_index;

    if (option.baudrates_count <= 1)
    {
        tio_printf("No alternative baudrates configured");
        return;
    }

    if ((old_index < 0) || (old_index >= option.baudrates_count))
    {
        old_index = 0;
    }

    new_index = (old_index + 1) % option.baudrates_count;

    if (tty_baudrate_apply(option.baudrates[new_index]) == 0)
    {
        option.baudrate_index = new_index;
        if (connected)
        {
            tcflush(device_fd, TCIFLUSH);
        }
        tio_printf("Switched baudrate: %d -> %d", old_baudrate, option.baudrate);
    }
    else
    {
        option.baudrate_index = old_index;
        if (tty_baudrate_apply(old_baudrate) != 0)
        {
            tio_warning_printf("Could not restore baudrate %d", old_baudrate);
        }
    }
}
        ...
        *forward = false;
        ...
        switch (input_char)
        {
            case KEY_QUESTION:
                ...
                tio_printf(" ctrl-%c a       Toggle auto baud switch", option.prefix_key);
                tio_printf(" ctrl-%c b       Send break", option.prefix_key);
                tio_printf(" ctrl-%c B       Cycle to next configured baudrate", option.prefix_key);
            ...
            case KEY_A:
                if (option.baudrates_count <= 1)
                {
                    ...
                    tio_printf("Auto baud switch unavailable");
                }
                else
                {
                    option.auto_baud_enabled = !option.auto_baud_enabled;
                    auto_baud_sample_count = 0;
                    auto_baud_failed_probe_pending = false;
                    tio_printf("Switched auto baud switch %s", option.auto_baud_enabled ? "on" : "off");
                }
                break;
...
static bool tty_baudrate_get_speed(int baudrate_value, speed_t *speed)
{
    speed_t baudrate;
...
bool tty_baudrate_is_supported(int baudrate)
{
    ...
    return true;
#else
    return tty_baudrate_get_speed(baudrate, NULL);
#endif
}
...
void tty_configure(void)
{
    ...
    /* Set speed */
    standard_baudrate = tty_baudrate_get_speed(option.baudrate, &baudrate);
    if (!standard_baudrate)
    {
#if defined (HAVE_TERMIOS2) || defined (HAVE_IOSSIOSPEED)
        // Use platform specific arbitrary baudrate support.
#else
...
static int tty_baudrate_apply(int baudrate)
{
    ...
    if (!standard_baudrate)
    {
        struct termios tio_current;

        if (tcgetattr(device_fd, &tio_current) == 0)
        {
            /* OS X wants these fields left alone before setting arbitrary baud rate */
            tio.c_ispeed = tio_current.c_ispeed;
            tio.c_ospeed = tio_current.c_ospeed;
        }
    }
#endif

    if (tcsetattr(device_fd, TCSANOW, &tio) == -1)
    {
        tio_warning_printf("Could not apply port settings (%s)", strerror(errno));
        return -1;
    }
    ...
    if (!standard_baudrate)
    {
        ...
        {
            tio_warning_printf("Could not set baudrate speed (%s)", strerror(errno));
            return -1;
        }
    }
...
{
    tty_baudrate_apply(option.baudrate);
}
...
static readability_t tty_sample_readability(const unsigned char *buffer, size_t length)
{
    size_t good = 0;
    size_t bad = 0;

    if (length < AUTO_BAUD_MIN_SAMPLE_SIZE)
    {
        return READABILITY_INCONCLUSIVE;
    }
    ...
    for (size_t i = 0; i < length; i++)
    {
        ...
        {
            const gchar *p = (const gchar *)&buffer[i];
            gunichar utf8_char = g_utf8_get_char_validated(p, length - i);

            if ((utf8_char != (gunichar)-1) && (utf8_char != (gunichar)-2))
            {
                size_t sequence_length = g_utf8_next_char(p) - p;
                good += sequence_length;
                i += sequence_length - 1;
            }
            else
            {
    ...
    if ((bad >= AUTO_BAUD_MIN_BAD_BYTES) && ((bad * 100 / length) >= AUTO_BAUD_GARBLED_THRESHOLD_PCT))
    {
        return READABILITY_GARBLED;
    }

    if ((good * 100 / length) >= AUTO_BAUD_READABLE_THRESHOLD_PCT)
    {
...
static void tty_auto_baud_sample_reset(void)
{
    auto_baud_sample_count = 0;
}

static long timeval_elapsed_ms(struct timeval *start)
{
...
static bool tty_auto_baud_cooldown_active(void)
{
    if (!auto_baud_failed_probe_pending)
    {
        return false;
    }

    if (timeval_elapsed_ms(&auto_baud_failed_probe_at) < AUTO_BAUD_FAILED_PROBE_COOLDOWN_MS)
    {
        return true;
    }
...
static bool tty_auto_baud_active(void)
{
    return option.auto_baud_enabled &&
           (option.baudrates_count > 1) &&
           (option.output_mode == OUTPUT_MODE_NORMAL) &&
           interactive_mode;
}

static bool tty_auto_baud_observe(const unsigned char *buffer, size_t length)
{
    size_t offset = 0;

    if (!tty_auto_baud_active())
    {
        return false;
    }

    if (tty_auto_baud_cooldown_active())
    {
        /* Start fresh after cooldown so one noisy burst cannot keep extending itself. */
        tty_auto_baud_sample_reset();
        return false;
    }

    while (offset < length)
    {
        size_t space = sizeof(auto_baud_sample) - auto_baud_sample_count;
        size_t count = MIN(length - offset, space);
        readability_t readability;

        memcpy(auto_baud_sample + auto_baud_sample_count, buffer + offset, count);
        auto_baud_sample_count += count;
        offset += count;

        readability = tty_sample_readability(auto_baud_sample, auto_baud_sample_count);
        if (readability == READABILITY_GARBLED)
        {
            tty_auto_baud_sample_reset();
            return true;
        }

        if (auto_baud_sample_count == sizeof(auto_baud_sample))
        {
            tty_auto_baud_sample_reset();
        }
    }
...
static ssize_t tty_auto_baud_read_probe(unsigned char *buffer, size_t length, int timeout_ms)
{
    ...
    while ((total < (ssize_t)length) && (timeval_elapsed_ms(&start) < timeout_ms))
    {
        long remaining = timeout_ms - timeval_elapsed_ms(&start);
        int timeout = remaining > 0 ? remaining : 0;
        int status = read_poll(device_fd, buffer + total, length - total, timeout);
        ...
        if (tty_sample_readability(buffer, total) != READABILITY_INCONCLUSIVE)
        {
            break;
        }
    }
...
static void tty_process_input(const unsigned char *input_buffer, ssize_t bytes_read, bool *do_timestamp)
{
...
static bool tty_auto_baud_probe(bool *do_timestamp)
{
    unsigned char probe_buffer[AUTO_BAUD_PROBE_SAMPLE_SIZE];
    struct timeval probe_start;
    int old_baudrate = option.baudrate;
    int old_index = option.baudrate_index;
    ...
    for (int i = 1; i < option.baudrates_count; i++)
    {
        int index = (old_index + i) % option.baudrates_count;
        int baudrate = option.baudrates[index];
        long remaining;
        ssize_t bytes_read;
        ...
        remaining = AUTO_BAUD_PROBE_TIMEOUT_MS - timeval_elapsed_ms(&probe_start);
        if (remaining <= 0)
        {
            break;
        }

        if (tty_baudrate_apply(baudrate) != 0)
        {
            break;
        }

        option.baudrate_index = index;
        tcflush(device_fd, TCIFLUSH);

        remaining = AUTO_BAUD_PROBE_TIMEOUT_MS - timeval_elapsed_ms(&probe_start);
        bytes_read = tty_auto_baud_read_probe(probe_buffer, sizeof(probe_buffer), remaining);
        if (bytes_read < 0)
        {
            break;
        }

        if (tty_sample_readability(probe_buffer, bytes_read) == READABILITY_READABLE)
        {
            tio_printf("AUTO BAUD CHANGED: %d -> %d", old_baudrate, baudrate);
            tty_process_input(probe_buffer, bytes_read, do_timestamp);
            tty_auto_baud_sample_reset();
            auto_baud_failed_probe_pending = false;
            return true;
        }
    ...
    tcflush(device_fd, TCIFLUSH);
    tty_auto_baud_sample_reset();
    gettimeofday(&auto_baud_failed_probe_at, NULL);
    auto_baud_failed_probe_pending = true;
    ...
    while (true)
    {
        ...
        if (status > 0)
        {
            ...
            if (FD_ISSET(device_fd, &rdfs))
            {
                ...
                if (tty_auto_baud_observe(input_buffer, bytes_read))
                {
                    if (tty_auto_baud_probe(&do_timestamp))
                    {
                        continue;
                    }
                }
                ...
                tty_process_input(input_buffer, bytes_read, &do_timestamp);
            }
